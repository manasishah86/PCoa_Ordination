...
normalize <- function (physeq, method = "tss", log = FALSE, returnMatrix = FALSE, 
    ...) 
{
    METHODS <- c("css", "tss", "deseq", "tmm", "upperquantile", 
        "vst", "pa")
    method <- pmatch(tolower(method), METHODS)
    if (is.na(method)) 
        stop("invalid normalization method")
    if (method == -1) 
        stop("ambiguous transformation method")
    if (method == "1") {
        normalizedObject = make_metagenomeSeq(physeq, ...)
        nmat = MRcounts(normalizedObject, log = FALSE, norm = TRUE, 
            sl = median(normFactors(normalizedObject)))
    }
    ...
    if (method == "3") {
        geoMeans = apply(counts, 1, gm_mean, na.rm = TRUE, zero.propagate = FALSE)
        deseq_sf = estimateSizeFactorsForMatrix(counts, geoMeans = geoMeans, 
            ...)
    ...
    if (log) {
        if (!method %in% c("6", "7")) {
            ...
            warning("Log transformation not applicable for method")
            return(NULL)
        }
    }
    if (method == "6") {
        ...
        geoMeans = apply(counts(dds), 1, gm_mean, na.rm = TRUE, 
    ...
    if (returnMatrix == FALSE) {
        physeq@otu_table = otu_table(nmat, taxa_are_rows = T)
        return(physeq)
    }
    else {
        return(nmat)
